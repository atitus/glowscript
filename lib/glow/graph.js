;(function () {
    ...
    function graph(options) {
        ...
        */
		...
		this.events = [] // each element looks like ['dataselected, plotly_click, etc.', function]
    ...
    property.declare( graph.prototype, {
        ...
        trigger: async function( type, evarg ) {
            var ev = {type:type, event:evarg}
            console.log(ev)
            for (var i=0; i<this.events.length; i++) {
                // this.events element is ['dataselected etc.', bound function]
                if (this.events[i][0] == type) {
                    (await this.events[i][1])(ev.event.x, ev.event.y) // call the bound function
                }
            }
        },
						...
						if (i === 0) {
							if (p == 'bar') Plotly.newPlot(this.__id, [ {x:[], y:[], type:p,
								...
								*/
								...
								s.__plotlyshaded_div = myPlot

								//begin plotly_selected event
...
// 											Plotly.relayout(myPlot, layoutupdate)
											...
											//delete old traces for selected data
										...
										var ev = {type:'dataselected', x:x, y:y}
										graph.trigger('dataselected',ev)
									}
								}) //end plotly_selected event
								
								//begin plotly_click event
								myPlot.on('plotly_click', function(eventData){
									...
									if(eventData !== undefined){
										...
										//call a bound function
										var ev = {type:'plotly_click', x:x, y:y}
										graph.trigger('plotly_click',ev)
									}
								}) //end plotly_click event
    ...
    function gobject(options) {
        ...
        this.__plotlyshaded_div = null //Aaron Titus
        var fast = true
        if (options.fast !== undefined){
        	fast = options.fast
        	delete options.fast
        ...
        */
        ...
        this.remove = function() { // JavaScript forbids anything.delete(), so in preprocessing we change .delete to .remove
        	...
        	var myPlot=this.__plotlyshaded_div
			//delete old traces for selected data
			for (var t=0; t< myPlot.data.length; t++){
				if(myPlot.data[t]['name']=='selected data'){
					Plotly.deleteTraces(myPlot, t);
